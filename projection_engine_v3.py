...
class ActiveRosterProvider:
    """Load current active roster names from official PLL team roster pages.

    The projection engine still uses historical stat rows for player ratings;
    this provider only decides which historical players are still eligible for
    the current depth chart.  It is intentionally defensive: failed network
    calls, page-shape changes, or tiny parse counts are ignored rather than
    breaking the app.
    """
        ...
        for team_id, urls in TEAM_ROSTER_URLS.items():
            ...
            for url in urls:
                try:
                    ...
                    logger.debug("Roster fetch/parse failed for %s %s: %s", team_id, url, exc)
                    continue
            if names:
                out[team_id] = names
                self.source_notes[team_id] = used_url or "unknown"
    ...
    def _load_rosters_from_csv(self) -> Dict[str, Set[str]]:
        ...
        for fp in candidates:
            ...
            try:
                df = pd.read_csv(fp)
                team_col = next((c for c in df.columns if c.lower() in {"team_id", "team", "teamid"}), None)
                name_col = next((c for c in df.columns if c.lower() in {"full_name", "player", "player_name", "name"}), None)
                if not team_col or not name_col:
                    continue
                rosters: Dict[str, Set[str]] = {}
                for _, r in df.dropna(subset=[team_col, name_col]).iterrows():
                    tid = str(r[team_col]).strip().upper()
                    key = _name_key(str(r[name_col]))
                    if tid and key:
                        rosters.setdefault(tid, set()).add(key)
                rosters = {tid: vals for tid, vals in rosters.items() if len(vals) >= self.min_players}
                if rosters:
                    logger.info("Active rosters loaded from CSV: %s", fp)
                    return rosters
            except Exception as exc:
                logger.warning("Could not load active roster CSV %s: %s", fp, exc)
        return {}
    ...
    def _extract_names_from_html(self, html: str) -> Set[str]:
        ...
        # 1) Try visible anchor text and player-profile links.
        try:
        ...
        # 2) Next/React-style JSON fields: firstName/lastName in either order.
        json_patterns = [
        ...
        # 3) Slug fields: "slug":"levi-anderson".
...
class ProjectionEngine:
    ...
    def fit(self, run_backtest: bool = False) -> Optional[BacktestResult]:
        ...
        self.current_rosters = ActiveRosterProvider().load_current_rosters()
