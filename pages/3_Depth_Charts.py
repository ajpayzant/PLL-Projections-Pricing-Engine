...
st.info(
    ...
    "The model now tries to filter each team to the official current PLL roster page first, "
    "then falls back to the latest-team stat-log method if a roster page cannot be parsed. "
    "If a player still looks wrong, use the Active toggle to remove them."
)

st.markdown("---")

current_rosters = getattr(engine, "current_rosters", {}) or {}
roster_source = getattr(engine, "current_roster_source", "unknown")
roster_error = getattr(engine, "current_roster_error", "")
if current_rosters:
    counts = {team_name(tid): len(vals) for tid, vals in current_rosters.items()}
    st.markdown(
        f'<span class="note-text">Official roster filter loaded for {len(current_rosters)} teams '
        f'via <b>{roster_source}</b>. Players not matched to the current roster are excluded before manual toggles.</span>',
        unsafe_allow_html=True,
    )
    with st.expander("Roster filter details", expanded=False):
        st.dataframe(
            pd.DataFrame([
                {"Team": tm, "Matched current roster names": n}
    ...
    detail = f" Reason: {roster_error}" if roster_error else ""
    st.markdown(
        f'<span class="note-text">Official roster filter unavailable; using latest-team stat-log fallback. '
        f'Source status: <b>{roster_source}</b>.{detail}</span>',
        unsafe_allow_html=True,
    )
