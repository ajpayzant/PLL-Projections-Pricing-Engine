...
with st.sidebar:
    ...
    ov_line   = st.number_input("Line", 0.0, 25.0, 0.5, 0.5, key="ov_line")
    ...
    with st.expander(
        ...
        if show_alt:
            for stat in stat_list:
                ...
                # Build a range of lines around the model projection
                max_val = float(np.percentile(dist, 95)) + 2.0
                alt_lines = [round(l * 0.5, 1) for l in range(1, int(max_val * 2) + 2)]
                alt_rows = []
                for al in alt_lines:
                    if al <= 0:
                        continue
